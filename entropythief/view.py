...
class Display:
    ...
    class Splash:
        ...
        _parent_display = (
            None  # retains a reference to the window _splashed i.e. Display
        )
        ...
        def __init__(self, display):
            ...
            nlines = coords[4] - coords[2]
            ncols = coords[5] - coords[3]
    ...
    def __init__(self, background):
        ...
        txt = """EntropyThief >cmd reference
>set buflim=<NUM>
>set maxworkers=<NUM>
>set budget=<FLOAT>
>stop       # exit or CTRL-C
...
"""
    ...
    def _load_background(self):
        ...
        inner_height, inner_width = (
            self.background.inner_height,
            self.background.inner_width,
        )
...
class View:
    ...
    def getinput(
        ...
        if whether_paused:
            ...
            msg.append(("  ",))
        msg.append(
            ("ESC", curses.A_ITALIC | curses.A_STANDOUT),
        )
        msg.append(("  ",))
        msg.append(
            (
        ...
        length_of_status_msg = functools.reduce(
            lambda a, b: a + b, [len(e[0]) for e in msg]
        )
        ...
        if curses.ascii.isascii(result):
            if result == 127:
                ...
                if len(self.linebuf) < maxX - 2:  # and len(self.linebuf) > 0:
                    ...
                    self.winbox.addstr(
                        0, len(self.linebuf), self.linebuf[-1]
                    )  # append last character from linebuf
                else:
