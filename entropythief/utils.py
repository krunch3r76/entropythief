...
def build_parser(description: str):
    ...
    parser.add_argument("--payment-driver", default="erc20", help="Payment driver name, `erc20`, `zksync`; default: \033[1m%(default)s\033[0m")
    parser.add_argument("--payment-network", default="rinkeby", help="Network name, e.g. `mainnet`, `rinkeby`, `goerli`, `polygon`, `mumbai`; default: \033[1m%(default)s\033[0m")
    parser.add_argument(
        "--subnet-tag", default="devnet-beta", help="Subnet name e.g. `devnet-beta`, `public-beta`; default: \033[1m%(default)s\033[0m"
    )
    parser.add_argument(
        "--log-file",
    ...
    parser.add_argument("--disable-logging", action="store_true", help="log to the stderr and the above log-file")
    parser.add_argument("--use-dev-random", action="store_true", help="use /dev/random instead of rdrand")
    parser.add_argument("--start-paused", action="store_true", help="start paused instead of beginning execution immediately")
    return parser
